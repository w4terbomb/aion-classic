...
public class QUEST_Q20109 extends QuestHandler
{
	private final static int questId = 20109;
	private final static int[] npcs = {701414, 798385, 800714, 800716};
	...
	public void register() {
		...
		qe.registerOnEnterZone(ZoneName.get("IDLDF1_SENSORY_AREA_Q20109_220080000"), questId);
	}
    ...
    public boolean onLvlUpEvent(final QuestEnv env) {
        ...
        if (player == null || env == null) {
            return false;
        } if (player.getRace() == Race.ASMODIANS && player.getLevel() >= 18 && player.getLevel() <= 20) {
    ...
    public boolean onEnterZoneEvent(QuestEnv env, ZoneName zoneName) {
        ...
        if (qs != null && qs.getStatus() == QuestStatus.START) {
            int var = qs.getQuestVarById(0);
			if (zoneName == ZoneName.get("IDLDF1_SENSORY_AREA_Q20109_220080000")) {
				if (var == 2) {
					qs.setQuestVar(3);
					updateQuestStatus(env);
					return true;
