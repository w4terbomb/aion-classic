...
public class _2031Petrifying_Elim extends QuestHandler
{
	...
	public void register() {
		...
		qe.registerQuestItem(182204002, questId);
		for (int npc: npcs) {
	...
	public HandlerResult onItemUseEvent(final QuestEnv env, Item item) {
		...
		final QuestState qs = player.getQuestStateList().getQuestState(questId);
		if (id != 182204002 || qs.getQuestVarById(0) != 4) {
			return HandlerResult.UNKNOWN;
		} if (!player.isInsideZone(ZoneName.get("HILL_OF_BELEMU_220020000"))) {
			return HandlerResult.UNKNOWN;
		}
		...
		ThreadPoolManager.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				removeQuestItem(env, 182204002, 1);
