...
public class MotionLoggingService {
	...
	public void createFinalFile() {
		...
		for (Entry<String, List<WeaponTime>> entry : results.entrySet()) {
			...
			listofWeapons.add(new WeaponTypeWrapper(WeaponType.BLADE_2H, null));
			//create MotionTime
	...
	private int recalculate(String method, WeaponTypeWrapper weapon, int attackSpeed, int time) {
		...
		float sword_bow_blade[] = {1200f,2400f};
		list.put(WeaponType.BOW, sword_bow_blade);
		list.put(WeaponType.SWORD_2H, sword_bow_blade);
		list.put(WeaponType.BLADE_2H, sword_bow_blade);
