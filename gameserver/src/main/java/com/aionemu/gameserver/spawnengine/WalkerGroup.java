...
public class WalkerGroup
{
	...
	public WalkerGroup(List<ClusteredNpc> members) {
		this.members = members.stream().sorted(Comparator.comparing(ClusteredNpc::getWalkerIndex)).collect(Collectors.toList());
	...
	public void form() {
        if (this.getWalkType() == WalkerGroupType.SQUARE) {
			...
			if (Arrays.stream(rows).reduce(0, Integer::sum) != members.size()) {
				log.warn("Invalid row sizes for walk cluster " + members.get(0).getWalkTemplate().getRouteId());
			}
			if (rows.length == 1) {
				float distance = (1 - members.size()) / 2f * (WalkerGroupShift.DISTANCE);
