...
public class ItemStorage
{
    ...
    public List<Item> getSpecialCubeItems() {
        return items.values().stream().filter(t -> t.getItemTemplate().getExtraInventoryId() > 0).collect(Collectors.toList());
    }

    public List<Item> getCubeItems() {
        return items.values().stream().filter(t -> t.getItemTemplate().getExtraInventoryId() < 1).collect(Collectors.toList());
    }
